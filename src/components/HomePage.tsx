...
interface HomePageProps {
  ...
  siteSettings?: {
    ...
    homepageContent?: {
      ...
      brandLogos?: Array<{ name: string; logoUrl: string }>;
...
export function HomePage({ onGetStarted, onLogin, siteSettings, onNavigate }: HomePageProps) {
  ...
  const siteName = settings.siteName || 'Fleet Guard 360';
    ...
    : [
      {
        ...
        answer: 'Fleet Guard 360 provides a centralized dashboard to track all your vehicles, their documents, service history, and upcoming renewals. You get smart reminders before any document expires or service is due.',
      },
      {
      ...
      },
      {
        ...
        answer: 'Yes! Fleet Guard 360 supports both private and commercial vehicles with different document tracking requirements for each type.',
      },
      {
      ...
      <section className="relative z-10 py-12 border-y border-gray-800 bg-gray-900/50 backdrop-blur-sm">
        <div className="max-w-7xl mx-auto px-4">
          <p className="text-center text-gray-500 text-sm mb-8">
            Trusted by fleet operators managing these brands
          </p>

          {(() => {
            const brandLogos: Array<{ name: string; logoUrl: string }> =
              ...
              <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-7 gap-6 items-center justify-items-center">
                {brandLogos.map((brand, i) => (
                  <div
                    ...
                    title={brand.name}
                  >
                    <div className="h-10 sm:h-12 w-full max-w-[140px] flex items-center justify-center">
                      <img
                        src={brand.logoUrl}
                        alt={brand.name}
                        ...
                        loading="lazy"
                        onError={(e) => {
                          // fallback to name text if image fails
                          (e.currentTarget as HTMLImageElement).style.display = 'none';
                        }}
                      />
                      <span className="text-sm font-semibold text-gray-500 hidden">
                        {brand.name}
                      </span>
                    </div>
                  </div>
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div>
              ...
              <p className="text-gray-400 text-lg mb-8">
                Whether you manage bikes, cars, trucks, or a mixed fleet - Fleet Guard 360 handles it all.
